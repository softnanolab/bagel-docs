...
- Merging to `main` triggers the Docker image workflow, which builds a temporary `sha-<commit>` validation tag, verifies it, and then promotes it to `latest` and the current package version on Docker Hub.

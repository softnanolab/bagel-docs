...
The `parse_backend()` method in `ModelWrapper` handles the `"apptainer:tag"` syntax, splitting the string to extract a container image tag. When you omit the tag, the runtime uses the installed boileroom package version.
